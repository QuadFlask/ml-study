...
# 4 layers (3 conv, 1 dense) with ensemble Accuracy:  0 Accuracy: 0.99 , 1 Accuracy: 0.9883
...
with tf.Session() as sess:
    ...
    print('Learning start', time.asctime(time.gmtime()))
    ...
    ensemble_correction_prediction = tf.equal(tf.argmax(predictions, 1), tf.argmax(mnist.test.labels, 1))
    ensemble_accuracy = tf.reduce_mean(tf.cast(ensemble_correction_prediction, tf.float32))
